  ...
  <div>
    ...
    <el-card shadow="never">
      ...
      <div class="stats-grid">
        <div class="stat-item">SL tài khoản: <b>{{ stats.accounts }}</b></div>
        <div class="stat-item">SL gói cước đã mua: <b>{{ stats.plansBought }}</b></div>
        <div class="stat-item">SL lượt đã ký: <b>{{ stats.signings.toLocaleString('vi-VN') }}</b></div>
        <div class="stat-item">SL CTS cá nhân: <b>{{ stats.ctsIndividual }}</b></div>
        <div class="stat-item">SL CTS tổ chức: <b>{{ stats.ctsOrg }}</b></div>
        <div class="stat-item">SL CTS cá nhân thuộc tổ chức: <b>{{ stats.ctsIndividualOfOrg }}</b></div>
      </div>
      ...
      <el-table :data="pagedList" v-loading="loading" border>
        ...
        <el-table-column prop="ctsType" sortable width="155">
          <template #header>
            ...
            <div class="col-filter">
              <el-select v-model="filterCtsType" size="small" clearable placeholder="" style="width:100%" @change="page = 1">
                <el-option label="Cá nhân" value="INDIVIDUAL" />
                <el-option label="Tổ chức" value="ORGANIZATION" />
                <el-option label="Cá nhân thuộc tổ chức" value="INDIVIDUAL_OF_ORG" />
              </el-select>
            </div>
          </template>
          <template #default="{ row }">{{ ctsTypeLabel(row.ctsType) }}</template>
        </el-table-column>
        ...
        <el-table-column prop="ctsDuration" sortable width="130">
          <template #header>
            ...
            <div class="col-filter">
              <el-select v-model="filterCtsDuration" size="small" clearable placeholder="" style="width:100%" @change="page = 1">
        ...
        <el-table-column prop="ctsStatus" sortable width="145">
          <template #header>
            ...
            <div class="col-filter">
              <el-select v-model="filterCtsStatus" size="small" clearable placeholder="" style="width:100%" @change="page = 1">
          ...
          <template #default="{ row }">
            <el-tag v-if="row.ctsStatus === 'PENDING_ACTIVATE'" type="primary" size="small" effect="plain">Chờ kích hoạt</el-tag>
            <el-tag v-else-if="row.ctsStatus === 'REVOKED'" type="info" size="small" effect="plain">Đã thu hồi</el-tag>
            <span v-else :class="ctsStatusClass(row.ctsStatus)">{{ ctsStatusLabel(row.ctsStatus) }}</span>
          </template>
        </el-table-column>
        ...
        <el-table-column prop="signings" sortable width="120" align="right">
          ...
          <template #default="{ row }">{{ row.signings.toLocaleString('vi-VN') }}</template>
        </el-table-column>
        ...
        <el-table-column prop="plan" sortable min-width="145">
          <template #header>
            ...
            <div class="col-filter">
              <el-select v-model="filterPlan" size="small" clearable placeholder="" style="width:100%" @change="page = 1">
        ...
        <el-table-column prop="fee" sortable width="120" align="right">
          ...
          <template #default="{ row }">{{ row.fee.toLocaleString('vi-VN') }}</template>
        </el-table-column>
      </el-table>
...
type CtsStatus = 'ACTIVE' | 'PENDING_ACTIVATE' | 'PENDING_APPROVE' | 'REVOKED' | 'EXPIRED'
...
const CTS_TYPES: CtsType[] = ['INDIVIDUAL', 'ORGANIZATION', 'INDIVIDUAL_OF_ORG', 'INDIVIDUAL_OF_ORG', 'INDIVIDUAL_OF_ORG']
const CTS_STATUSES: CtsStatus[] = ['ACTIVE', 'PENDING_ACTIVATE', 'PENDING_APPROVE', 'REVOKED', 'EXPIRED', 'ACTIVE', 'ACTIVE', 'ACTIVE', 'ACTIVE', 'ACTIVE']
const CTS_DURATIONS = [1, 12, 24, 48, 1, 1, 1, 1, 1, 1]
const SIGNINGS = [3800, 0, 0, 150000, 100, 150000, 150000, 150000, 150000, 150000]
...
const filteredList = computed(() => {
  return list.value.filter(row => {
    if (filterCtsType.value && row.ctsType !== filterCtsType.value) return false
    if (filterCtsDuration.value && String(row.ctsDuration) !== filterCtsDuration.value) return false
    if (filterCtsStatus.value && row.ctsStatus !== filterCtsStatus.value) return false
...
.status-active  { color: #67c23a; font-size: 13px; }
.status-pending { color: #1B60CB; font-size: 13px; }
.status-inactive { color: #909399; font-size: 13px; }
</style>
