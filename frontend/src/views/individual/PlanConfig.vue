  ...
  <div>
      ...
      <el-table :data="pagedList" v-loading="loading" border>
        ...
        <el-table-column prop="status" sortable width="140">
          <template #header>
            ...
            <div class="col-filter">
              <el-select v-model="filterStatus" size="small" clearable placeholder="" style="width:100%" @change="page = 1">
          ...
          <template #default="{ row }">
            <el-tag :type="statusTagType(row.status)" :effect="row.status === 'APPLYING' ? 'dark' : 'light'" size="small">
        ...
        <el-table-column prop="applyFrom" sortable width="130">
          <template #header>
            ...
            <div class="col-filter">
              <el-date-picker v-model="filterApplyFrom" type="date" size="small" clearable placeholder="" style="width:100%" @change="page = 1" />
            </div>
          </template>
        ...
        <el-table-column prop="applyUntil" sortable width="130">
          <template #header>
            ...
            <div class="col-filter">
              <el-date-picker v-model="filterApplyUntil" type="date" size="small" clearable placeholder="" style="width:100%" @change="page = 1" />
            </div>
          </template>
        ...
        <el-table-column prop="updatedAt" sortable width="175">
          <template #header>
            ...
            <div class="col-filter">
              <el-date-picker v-model="filterUpdatedAt" type="date" size="small" clearable placeholder="" style="width:100%" @change="page = 1" />
            </div>
          </template>
        ...
        <el-table-column fixed="right" width="210">
          ...
          <template #default="{ row }">
            <div class="action-btns">
              <el-button size="small" :icon="Timer" @click.stop="goDetail(row)">Chi tiết</el-button>
              <el-button v-if="row.status === 'AVAILABLE'" size="small" type="primary" plain @click.stop="openRequestApply(row)">
                Y/c áp dụng
              </el-button>
              <el-button v-if="row.status === 'PENDING'" size="small" type="success" plain :icon="Check" @click.stop="openApprove(row)">
                Duyệt
              </el-button>
              <el-button v-if="row.status === 'APPROVED' || row.status === 'APPLYING'" size="small" type="primary" @click.stop="openStopApply(row)">
                Dừng áp dụng
              </el-button>
            </div>
          </template>
    ...
    <el-dialog v-model="requestApplyVisible" width="500px" align-center>
      ...
      <div class="dlg-name-row">
        <span>Tên: <b>{{ activeRow?.name }}</b></span>
        <el-button link type="primary" @click="goDetail(activeRow!)">Xem chi tiết</el-button>
      </div>
    ...
    <el-dialog v-model="approveVisible" width="500px" align-center>
      ...
      <p class="dlg-note">
        Khi chọn Xác nhận, gói cước sẽ chuyển sang trạng thái "Đã duyệt" và được áp dụng từ 00:00:00 ngày bắt đầu đến 23:59:59 ngày kết thúc.
      </p>
      <ul class="dlg-bullets">
        <li>Từ 00:00:00 ngày bắt đầu, trạng thái sẽ chuyển sang "Đang áp dụng"</li>
        <li>Sau khi hết thời gian hiệu lực, trạng thái sẽ tự động chuyển về "Khả dụng"</li>
        <li>Chọn Từ chối, trạng thái gói cước sẽ chuyển về "Khả dụng".</li>
      </ul>
      <template #footer>
        <div class="dlg-footer-split">
          <el-button type="warning" plain @click="confirmReject">Từ Chối</el-button>
...
const currentPlan = ref({ name: 'Chữ ký số 2026 tháng 3', applyUntil: '30/04/2026' })
const nextPlan = ref({ name: 'Chữ ký số 2026 tháng 5', applyFrom: '01/05/2026' })
...
const MOCK_DATA: PlanConfigRow[] = [
  { id: 1, name: 'Chữ ký số 2026', status: 'AVAILABLE', applyFrom: null, applyUntil: null, updatedAt: '27/03/2026 18:29:00' },
  { id: 2, name: 'Chữ ký số 2025', status: 'UNAVAILABLE', applyFrom: null, applyUntil: null, updatedAt: '27/03/2026 18:29:00' },
  { id: 3, name: 'Chữ ký số 2026 tháng 5_2', status: 'PENDING', applyFrom: '08/05/2026', applyUntil: '11/05/2026', updatedAt: '27/03/2026 18:29:00' },
  { id: 4, name: 'Chữ ký số 2026 tháng 5', status: 'APPROVED', applyFrom: '01/05/2026', applyUntil: '07/05/2026', updatedAt: '27/03/2026 18:29:00' },
  { id: 5, name: 'Chữ ký số 2026 tháng 3', status: 'APPLYING', applyFrom: '27/03/2026', applyUntil: '30/04/2026', updatedAt: '27/03/2026 18:29:00' },
  { id: 6, name: 'Chữ ký số 2025 tháng 12', status: 'UNAVAILABLE', applyFrom: '01/12/2025', applyUntil: '31/12/2025', updatedAt: '27/03/2026 18:29:00' },
  { id: 7, name: 'Chữ ký số 2025 tháng 6', status: 'UNAVAILABLE', applyFrom: '01/06/2025', applyUntil: '30/06/2025', updatedAt: '27/03/2026 18:29:00' },
  { id: 8, name: 'Chữ ký số 2024', status: 'UNAVAILABLE', applyFrom: null, applyUntil: null, updatedAt: '27/03/2026 18:29:00' },
]
...
function statusTagType(status: PlanStatus): '' | 'success' | 'warning' | 'info' | 'danger' {
  const map: Record<PlanStatus, '' | 'success' | 'warning' | 'info' | 'danger'> = {
    AVAILABLE: 'info',
    UNAVAILABLE: 'info',
    PENDING: 'warning',
    APPROVED: 'success',
...
.current-plan-info { margin-bottom: 12px; font-size: 14px; line-height: 1.8; color: #303133; }
...
.pagination-row { display: flex; align-items: center; justify-content: space-between; margin-bottom: 8px; }
...
.dlg-footer-right { display: flex; gap: 8px; }
</style>
