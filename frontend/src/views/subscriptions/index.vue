<template>
  <div>
    ...
    <el-card shadow="never">
      <div class="toolbar">
        <el-input v-model="searchQuery" :placeholder="t('subscriptions.searchPlaceholder')" clearable style="width:240px" @input="onSearchInput" />
      ...
      <el-table :data="list" v-loading="loading" stripe>
        ...
        <el-table-column :label="t('subscriptions.type')" width="100">
          <template #default="{ row }">
            <el-tag :type="row.subscriberType === 'INDIVIDUAL' ? 'primary' : 'warning'" size="small">
              {{ row.subscriberType === 'GROUP' ? 'PARTNER' : row.subscriberType }}
            </el-tag>
          </template>
        </el-table-column>
        <el-table-column :label="t('subscriptions.userPartner')" min-width="140">
          <template #default="{ row }">{{ row.userId || (t('subscriptions.partner') + ' #' + row.groupId) }}</template>
        </el-table-column>
...
import { listSubscriptions, cancelSubscription, suspendSubscription, reactivateSubscription, getAuditLog } from '@/api/subscriptions'
...
async function load() {
  loading.value = true
  try {
    const res = await listSubscriptions({ status: filterStatus.value || undefined, query: searchQuery.value || undefined, page: page.value - 1, size: size.value })
